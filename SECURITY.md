...
- 使用 HTTPS 反向代理；
- 首发关闭手机号注册和短信验证码；如开放非手机号账号/密码自助注册，必须默认低权限、限制资源使用，并由管理员手动开通高级导出；
- 限制服务器、容器和数据卷访问权限；
- 定期备份并演练恢复。
...
后续公网内测后台应把导出总开关扩展为多个可组合授权项，至少区分基础技术汇总、逐小时明细、经济汇总、经济详表/年度现金流、图表包和报告下载。任何新增下载项都必须先接入后端权限校验和审计，再暴露前端按钮。
