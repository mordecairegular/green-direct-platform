...
10. 提供内部试用后台的本地账号、认证、可选登录门禁、项目工作区门禁、最小平台账号/项目成员管理页、权限、任务和结果存储服务骨架，以及 `pilot-admin` 命令行账号管理入口。
...
该 CLI、登录门禁、项目工作区和最小平台管理页是内部试用阶段的本地账号/项目控制入口，不代表正式公网 SaaS 身份系统、数据库会话或后台任务队列已完成。
