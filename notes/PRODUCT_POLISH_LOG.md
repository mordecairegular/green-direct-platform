...
### 2026-07-02 推荐页三维经济性图交互与政策状态分层

用户指出三维经济性图已接近预期，但默认视角不易查看空间内部方案，未达政策指标的方案应弱化区分，当前查看方案也需要更明显的空间定位。

决策：
- 经济性三维图启用 Plotly 3D 工具栏、滚轮缩放和 orbit 拖拽，保留用户旋转后的相机状态，便于查看内部候选点。
- 候选方案按政策状态分层展示：达标候选继续作为主图层并按储能容量着色；未达政策候选使用灰色 `x` 符号、较低透明度和较小点尺寸，保留悬浮复核能力但不抢主视觉。
- 当前查看方案新增红色光环和三条投影定位线；推荐组合仍使用红色菱形，避免把“推荐席位”和“正在查看”混为同一含义。

边界：
- 本轮仍只调整图表交互和视觉编码，不改变政策过滤、推荐排序、经济性计算或候选方案结果。

### 2026-07-02 推荐页经济指标统一展示与三维对比图收口

用户指出推荐方案卡片只展示命中席位对应的一项经济指标，不利于横向比较；经济性对比图的颜色、大小、图例含义混杂，且只看风电或储能单变量变化不足以解释风光储组合变化。

决策：
- 04 方案推荐页的每张推荐卡片统一展示三项经济指标：同一主体 FIRR、电源侧 FIRR、负荷侧收益；缺失项显示为空值占位，不再按推荐席位只挑一项展示。
- 经济性对比图从双二维散点改为三维散点：X 轴为光伏容量，Y 轴为风电容量，Z 轴为电源侧 FIRR，颜色仅表示储能容量；推荐组合用红色菱形单独标识。
- 图例不再把 FIRR 作为点图例重复表达，FIRR 只保留在 Z 轴和悬浮提示中；点大小固定，避免再引入未解释的视觉编码。

边界：
- 本轮不改变经济性现金流、FIRR 计算、推荐排序和候选方案生成，只调整推荐结果展示和图表编码方式。
...
### 2026-06-17 方案数预估轴计数与枚举固定开销优化

本轮继续处理“成千上万个方案等待很久”的前置性能问题。此前 UI 和 `run_batch()` 已有方案数提醒、硬上限和 summary-first，但 `estimate_scenario_count()` 曾通过 `generate_scenarios()` 完整构造 `Scenario` 列表来计数；第一步已改成 count-only，第二步继续把 count-only 收紧为轴计数，不再展开光伏容量轴 × 风电容量轴。当容量范围设置过大时，即使最终要被硬上限拒绝，系统也可以快速得到理论方案数并给出拦截。

实现：
- `scenario_generator.py` 新增轴计数 helper，按容量轴的负/零/正数量推导候选组合数，不构造 `Scenario` 对象，也不展开光伏 × 风电笛卡尔积；
- `estimate_scenario_count()` 改为调用 `count_scenarios()`，用于 UI 方案数预估；
- `run_batch()` 先用轴计数结果检查 `PerformanceParams.max_scenarios_per_run`，超限时直接拒绝，不再先生成完整方案池；
- `generate_scenarios()` 预先计算光伏容量轴、风电容量轴和合法储能功率/时长组合，保持原 `scenario_id` 顺序和过滤规则不变；
- 新增测试覆盖 count-only 路径、轴计数不展开容量轴、计数与真实生成数量一致，以及超限拒绝发生在方案池物化之前。
...
小基准：
- 25,596 个候选方案样本，优化前 `estimate_scenario_count()` 约 0.0407s，轴计数后约 0.000052s；
- 同一样本，`generate_scenarios()` 约从 0.0410s 降到 0.0296s；
- 理论 1,999,999,999,998 个候选组合的巨大样本只做预估约 0.000067s。

验证：
- `python -m pytest tests\test_batch_runner.py -q` 通过，16 项通过；
...
### 2026-06-23 桌面端浅色主题与原生帮助修复

用户反馈方案仿真、经济测算和图表概览在系统深色配色下出现控件文字不可读、问号重叠、悬浮说明失效和图表轴标注过浅等问题。

决策：

- 固定 Streamlit 应用为浅色主题，避免操作系统深色模式影响平台基础可读性；
- 不再自造问号图标或伪 tooltip，统一使用 Streamlit 原生 `help=` 命中区和悬浮说明；
- 经济测算输入标签保留业务名，常见单位放入原生 help，减少横向拥挤；
- 图表概览的核心对比热力图直接在 Plotly figure 内设置轴文字、色标和格内文字颜色，不用页面级 CSS 覆盖图表。

边界：

- 本次为 UI 可读性和布局修订，不改变 V0.1 技术仿真、经济性 V1 现金流、推荐排序或输出口径；
- 高级图表模块仍是早期原型，本次只修复当前首屏核心对比的可读性，不做大规模图表重构。

### 2026-06-29 首页启动台文案减负

用户在首页浏览器批注中指出：

- “项目启动台”标题下的解释性副标题不需要赘述；
- “项目边界”表更像开发人员或项目文档内容，不应作为平台首页模块。

决策：

- 首页标题区只保留“项目启动台”标题，不再显示解释性副标题；
- 首页保留面向使用者的准备度卡片和“下一步建议”，移除“项目边界”表；
- 项目边界、储能策略、推荐口径等开发/产品约束继续以项目文档为准，不放在首页主界面。

边界：

- 本次只调整首页信息架构和展示文案，不改变仿真、经济性、推荐、图表或导出口径。

### 2026-06-30 报告导出与到户电价口径收口

用户要求对完整版本进行收口，重点指出“到户电价展示”中下网到户价、绿电仍缴输配和绿电仍缴基金的关系容易被误读，并反馈“下载报告”模块存在 `bytes` 再 `.encode()` 的崩溃。

决策：

- 明确两条展示路径：外部购网路径使用“下网到户价”；自发自用绿电路径使用“绿电结算价 + 绿电仍缴输配 + 绿电仍缴基金”；绿电后综合到户价按下网电量和自发自用绿电量加权，三项不是直接相加；
- 固定价模式继续按 `绿电结算价 + 仍缴输配 + 仍缴基金` 计算绿电自用到户价；
- 逐时电价曲线模式新增识别可选 `green_direct_retained_transmission_distribution_tariff_with_vat` 和 `green_direct_retained_gov_fund_surcharge` 字段；若曲线未提供仍缴列，则沿用既有“输配/基金仍缴”的默认展示；
- 下载报告页新增“公文版 DOCX 报告模板”下载，模板写入当前方案技术指标、经济性摘要、到户电价公式和图表插图清单；PNG 图表包仍作为正式报告插图来源；
- 报告导出审计的文件大小统一通过 payload size helper 计算，避免 bytes payload 触发 `.encode()` 错误。

边界：

- 本轮不改变小时级技术调度、不改变推荐排序、不改变已实现经济性现金流的核心公式；
- DOCX 模板是平台报告结构和口径模板，不在模板内重复生成图表，正式报告图表仍从图表 PNG 包插入；
- “向 GPT 咨询”的需求未调用外部站点；当前内置浏览器打开的是本地平台页面，报告模板按平台数据结构、图表模块和中文正式报告写法直接设计。

### 2026-06-30 报告模板、默认方案和方案池布局继续收口

用户继续反馈：

- “图表下载和报告生成”中的默认方案应为推荐方案排序第一位；
- DOCX 报告不能只是简单模板，应基于平台已有图表自动嵌入关键图表；
- “方案仿真”的候选方案池高度仍偏高，应把方案数状态移到方案生成方式右侧空白区，并缩短“储能时长选项（小时）”标签。

决策：

- 导出页的默认报告/图表方案以推荐组合 portfolio 的第一条有效 `scenario_id` 为准；推荐结果变化时自动切换到新的推荐首位，同一结果下允许用户手动选择其他方案；
- DOCX 报告模板升级为“正文含关键图表”的报告模板：正文自动嵌入方案组合政策对比、容量对比、典型/关键日、全年运行、SOC、电网交换和月度能量流向等可生成 PNG；
- 全量图表、小时级明细、全量方案池、完整现金流和字段字典仍放在 Excel/ZIP/附录中，正文不承载全量明细；
- 方案候选池只保留一个“当前表单方案数”状态卡，放到“方案生成方式”右侧；底部重复的“光伏/风电配置”和“储能时长”统计卡删除；
- 储能时长输入标签改为“储能时长（h）”，完整解释放在原生 help 中。

边界：

- 本轮仍不改变小时级技术调度、不改变经济性现金流核心口径、不改变推荐排序算法；
- 如果本机缺少静态图导出环境导致某张 PNG 生成失败，DOCX 不崩溃，失败说明写入报告正文，HTML/Excel/CSV 导出仍可用于复核；
- 6 月 30 日上一条记录中“DOCX 不在模板内重复生成图表”的边界已被本条修订替代：现在 DOCX 正文嵌入关键图表，全量图表仍通过 ZIP 附录导出。

### 2026-06-30 导出页入口和侧边栏紧凑化修正

用户反馈：

- “下载报告”页面进入失败；
- 侧边栏顶部留白较多，14 寸显示屏上需要滚动才能看到完整信息；
- 经济测算输入参数布局仍显混乱，但要求先给优化建议或效果图，经确认后再动手。

决策：

- 导出页不再在页面进入阶段同步生成含图表 DOCX；DOCX 改为勾选“准备公文版 DOCX 报告（含图表）”后再渲染 PNG 和生成文档；
- 导出页默认仍提供轻量 Excel、Markdown 和图表包入口，避免静态图依赖或图表渲染耗时阻塞页面入口；
- 侧边栏压缩 Streamlit 原生 header、高度、品牌区、导航按钮、状态卡和曲线预览间距；隐藏曲线预览说明小字，只保留关键指标；
- 经济测算布局本轮只提出方案，不直接改代码。

边界：

- 本轮不改变导出内容口径，只改变重任务触发时机；
- 不改变经济测算字段、公式、默认值和现金流逻辑；
- 侧边栏压缩只作用于展示密度，不改变工作流页面和权限逻辑。

### 2026-07-01 用能侧结算口径命名收口

用户确认“到户电价展示”作为模块标题不够准确，改用“用能侧结算口径”。

决策：

- 03 经济测算页原“到户电价展示”输入区改名为“用能侧结算口径”；
- 03 经济测算结果卡片标题改为“用能侧结算对比”，05 图表概览方案组卡片标题改为“方案组用能侧结算对比”；
- Markdown 和 DOCX 报告中的对应章节改为“用能侧结算口径”，DOCX 表题改为“用能侧结算关系和当前方案展示值”；
- 具体价格指标仍保留“下网到户价”“绿电前/后综合到户价”“绿电自用到户价”等表述，因为这些是业务价格指标，不是模块标题。

边界：

- 本轮只改用户可见命名和报告标题，不改 `landed_price_*` 计算字段、公式、推荐排序或现金流口径；
- “下网到户价、绿电仍缴输配、绿电仍缴基金不是直接相加关系”的解释继续保留。

### 2026-07-01 经济性测算页面两块式布局落地

用户指出此前只改了命名，没有真正调整 03 经济性测算页布局。

决策：

- 03 页顶部改为紧凑“运行参数”条，`运营期`、`折现率`、`FIRR 门槛`三个输入项采用同行标签和输入框；
- 计算状态移到页面右上角，展示“计算状态、价格模式、方案池”，计算按钮置于状态面板下方；
- 主输入区改为两大块：左侧“建设投资与成本费用”，内含建设投资、成本费用和高级储能更换；右侧“收入和税金”，内含收益电价、税金、用能侧结算口径、高级其他经营收入和高级电费清单组价；
- 右侧“收益电价”在 14 寸视口下采用 2×2 布局，避免“外部购电净成本”等长标签被挤压换行；
- 03 页不再默认展示用能侧结算结果卡和推荐式核心结果，计算完成后只保留成功提示和折叠复核表；推荐展示继续放在 04 页。

边界：

- 本轮不改变 `EconomicParams`、`AvoidedGridPurchaseParams`、现金流、价格曲线和推荐排序计算口径；
- 经济性结果复核表仍保留在折叠区，满足审计和导出前复核需求；
- 浏览器验证覆盖默认桌面视口和 1366×768 近 14 寸视口。

### 2026-07-01 DOCX 报告模板多方案收口

用户反馈：

- 下载的 DOCX 图片段落继承固定行距，导致图片显示不完整；
- 报告图表缺少基于方案数据的业务解读；
- 模板只围绕单一方案展开，不符合实际需要对比 3-5 个代表方案的报告场景；
- 图表下载和报告生成页的默认方案应采用平台推荐的“电源侧 FIRR 最优/电源投资 IRR 最优”方案。

决策：

- DOCX 图片段落增加专用 `ImageCN` 样式，使用自动行距并尽量与图题保持连续，避免继承正文固定行距导致图片裁切；
- 公文版 DOCX 结构调整为“项目摘要与结论、代表方案组合、测算口径、技术指标对比、用能侧结算口径、经济性测算、图表复核、结论建议、附件建议”；
- 正文默认纳入最多 5 个代表方案，最终推荐方案置前，其余方案按推荐组合和对比组顺序进入；全量方案池、逐小时明细和现金流明细仍放在 Excel/ZIP 附件；
- DOCX 图表说明由泛化说明改为基于所选方案和对比组数据生成，包括政策指标、容量范围、典型日/关键日、全年运行、SOC、电网交换和月度能量流向；
- 下载报告默认方案优先读取推荐组合中的 `power_side_firr_best` 席位；若该席位不存在，再回退到推荐组合第一个有效方案和普通默认方案。

边界：

- 本轮不改变技术仿真、政策过滤、经济性现金流、推荐排序算法和用能侧结算公式；
- “下网路径到户价”和“绿电自用路径费用”继续作为两条不同用能侧结算路径解释，不把下网到户价与仍缴输配/基金直接相加；
- DOCX 正文只承载代表方案和关键图表，审计级全量数据仍通过导出包复核。

### 2026-07-01 DOCX 报告并网型收口与能流图复用

用户进一步明确：

- 当前软件报告只面向并网型绿电直连/源网荷储项目，不体现柴发和离网内容；
- DOCX 表格文字垂直居中、字体统一黑色；
- 代表方案组合汇总表按万千瓦、亿元等单位展示，风光储配置拆列，保留绿电占比、弃电率、自发自用比例、建设投资、IRR；
- 报告正文必须嵌入平台已有图表，包括能量流向、四季典型日、关键运行日和全年 8760/8784 曲线，并配简要业务解读；
- 完整候选方案池和推荐方案逐小时能量台账不再作为 DOCX 正文/附件内容，继续通过 Excel/ZIP 导出复核。

决策：

- 报告正文固定为并网型绿电直连/源网荷储测算报告边界，删除离网、柴油等后续扩展表达；
- 表格生成器统一写入单元格垂直居中和黑色字体属性，图片段落使用自动行距，避免截图裁切；
- 将图表概览页的年度能源流向 Sankey 抽象为 `build_energy_flow_chart`，网页端和 DOCX 导出共用同一平台图表构建函数；
- DOCX 图表顺序增加“推荐方案年度能源流向图”，并继续包含月度新能源流向、月度负荷来源、四季典型日、最大负荷日、最大弃电日、最大下网日、全年运行曲线、SOC 和电网交换图；
- 图下注释使用 summary/hourly_detail 中的方案指标生成，说明数据来源和必要的展示近似，不重新计算调度。

边界：

- 本轮不改变现有技术仿真、储能调度、经济性现金流、推荐排序和用能侧结算公式；
- 年度能流图中光伏/风电到各去向的细分仍按年度发电占比分摊展示，严格追溯以逐小时台账和 Excel/ZIP 导出为准；
- DOCX 保持“正文报告”定位，审计级全量表继续由下载页独立导出。

### 2026-07-01 DOCX 版式、曲线输入和推荐页经济性对比收口
用户继续反馈：
- DOCX 正文未两端对齐，章节标题需缩进 2 字符，长表表头拥挤，典型日/关键日和全年曲线存在疑似取图错误；
- 平台需明确负荷、光伏、风电曲线计量单位，且项目不包含光伏或风电时不应强制上传对应出力曲线；
- 方案推荐页需要新增不同方案经济性对比图，字段对应关系表的中文列不能继续裸露英文原名。

决策：
- DOCX 正文样式统一两端对齐，一级章节标题首行缩进 2 字符；代表方案组合汇总表单独放入横向页面，后续正文恢复纵向；
- DOCX 图表导出改为逐张渲染，典型日和关键运行日标题、图名和元数据写入实际选中日期；关键运行日选取尽量避免最大负荷、最大弃电、最大下网命中同一天；
- 静态图导出提高 SOC、电网交换和全年 8760/8784 曲线高度，减少插入报告后的压缩感；
- 方案仿真输入明确：负荷曲线为功率（万kW），光伏、风电曲线为单位容量出力系数（p.u.）；当候选方案中某类新能源容量全为 0 且用户未上传对应曲线时，平台自动补零出力系数曲线并写入技术输入快照；
- 方案推荐页新增“经济性对比图”，读取现有技术 summary、经济性 summary 和推荐组合，展示电源侧 FIRR 随风电容量和储能容量变化的趋势，并高亮当前推荐组合；
- 字段对应关系表补齐常见经济性字段中文名，未配置中文名时显示“未配置中文名”，不再在“中文名”列回退显示英文原名。

边界：
- 本轮不改变逐小时技术仿真、储能调度、经济性现金流、推荐排序和用能侧结算公式；
- 自动补零曲线只在该资源容量全为 0 时触发；只要候选池存在正容量，仍要求上传对应光伏/风电出力系数曲线；
- 经济性对比图是结果展示图，不引入新的优化、反解或推荐席位。
