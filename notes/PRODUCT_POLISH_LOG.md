...
### 2026-07-04 经济性页三栏输入细节压缩

用户确认 03 页“建设投资与成本费用 / 收入与税金 / 下网电费单”三栏平行布局基本可用，但指出局部标签过长会造成输入区拥挤。
随后用户指出，造价属于高频调整值，不能因为标签压缩和三栏布局把 `-` / `+` 微调能力丢掉；微调按钮应恢复，但不能拉长输入框或让按钮高度超过输入框。

决策：
- 经济性页下方输入区以三栏平行布局为准，不继续采用“下网电费单”独立满宽卡片作为默认形态。
- “其他固定资产投资”可见标签压缩为“其他固投”，完整含义放入问号帮助，避免在建设投资三列中换行。
- “企业所得税率”可见标签压缩为“所得税率”，问号帮助明确其口径为企业所得税率。
- “风电单位造价 / 光伏单位造价 / 储能单位造价”可见标签压缩为“风电造价 / 光伏造价 / 储能造价”，单位造价口径保留在问号帮助中。
- 三栏并排时，“下网电费单”内部按两列三行排列账单项，避免在窄卡片内继续使用三列导致拥挤。
- 高频造价和运维成本输入恢复紧凑 `-` / `+` 微调控件：数值框、减号和加号共用原字段列宽，按钮固定为与输入框同高的 34px，不额外拉长输入框。

边界：
- 本轮只调整可见标签、帮助说明和输入布局，不改变投资、税率、电费单推导、经济性计算、推荐排序或导出字段口径。

### 2026-07-04 经济性页电费单输入区拆分

用户指出“下网电费单”不应继续塞在“收入、税金与电费单”长卡片里，相关说明文字也不应占据正文高度；总体要求是让各个输入框更美观地填满页面。

决策：
- 03 页经济性输入区改为上排“建设投资与成本费用”“收入与税金”两张卡片，下排“下网电费单”独立满宽卡片。
- “下网电费单”六个账单项按三列两行铺满页面：到户综合电价、购电增值税率、线损单价、系统运行费、输配电价、政府基金及附加。
- 原说明“默认按电费单到户综合电价和可查账单项内部推导同一主体净节费；自发自用绿电默认仍缴输配电价和政府性基金及附加。起始值按湖南 110kV 工商业两部制用户常用账单项预填，请按项目电费单复核。”不再正文展示，改为“下网电费单”标题旁问号提示。

边界：
- 本轮只调整经济性页输入布局和说明层级，不改变湖南默认值、同一主体净节费推导、电源侧/同一主体经济性计算、推荐排序或导出字段。

### 2026-07-04 推荐卡片点击方式修正

用户指出“卡片按钮联动”仍然多了一层没有必要的控件，推荐方案卡片本身就是选择对象，应该直接点击卡片切换经济性对比图中的当前查看方案。

决策：
- 04 页推荐卡片不再额外渲染“切换到此方案 / 当前查看方案”按钮，整张推荐卡片直接作为点击目标。
- 点击卡片后通过 Streamlit 原生 widget 事件写入当前查看方案状态，卡片高亮和经济性 3D 图红色光环同步更新；不引入自定义前端 JS，也不通过 URL 跳转重建会话。
- 2026-07-04 用户进一步指出不应为了点击功能牺牲原卡片排版。已恢复原 `.gd-rec-card` HTML 展示结构，包括序号、标签、方案号、状态胶囊、容量、推荐理由和 3 列指标栅格；点击行为通过透明原生触发层实现，视觉上不再出现额外按钮。

边界：
- 本轮只修正卡片交互入口和点击层实现，不改变推荐排序、经济性计算、图表 hover 口径、卡片指标口径或导出默认方案。

### 2026-07-04 推荐卡片联动经济性对比图

用户指出 04 方案推荐页的“经济性对比图”虽然有红色光环标识当前查看方案，但页面上没有明确入口选择当前查看方案；上方推荐卡片只是展示信息，没有与图表联动。同时，图中的红色推荐菱形悬浮时不能说明是哪一个推荐方向，普通方案悬浮反馈也不够明显。

决策：
- 推荐卡片变成经济性对比图的选择入口：每张卡片提供“切换到此方案 / 当前查看方案”按钮，选中后该卡片高亮，并同步更新图中的红色光环和定位线。
- 新增推荐页专用当前查看方案 session 状态，优先级高于图表页和导出页的旧状态；选卡片时同时同步到图表页当前方案状态，便于后续进入图表概览继续查看同一方案。
- 经济性对比图的 hover 数据增加“推荐席位”字段；推荐组合红色菱形显示其命中的推荐方向，普通候选显示“普通候选”。
- 3D 图的 hover label 增强边框和背景色，达标候选、未达政策、推荐组合和当前查看方案保持不同视觉反馈；当前查看方案仍用红色光环和投影定位线，不与推荐席位标识混用。

边界：
- 本轮只改 04 页推荐卡片交互、图表 hover 和选中状态，不改变推荐算法、经济性计算、政策筛选、图表数据口径或导出范围。
- Plotly 3D 原生悬浮不会像自研 WebGL 组件那样在鼠标经过时动态放大单个点；当前用更明确的 tooltip、红色光环和卡片联动来解决识别问题。

### 2026-07-04 经济性测算去内部阶段名并增加计算反馈

用户指出“经济性 V1 计算”这类名称属于开发过程中的中间叫法，不应出现在对外版本；同时经济性测算点击后缺少明确运行反馈，用户会感觉页面卡住，等待几秒后屏幕闪烁并直接出结果。

决策：
- 03 页对外按钮、成功提示、说明文案和底部状态不再显示“V1”，统一称为“经济性测算”“计算经济性”“经济性已计算”。
- 内部状态键、历史口径文档和开发资料可继续保留 `economy_v1_result`、`推荐 V1` 等名称，避免为 UI 文案做高风险结构重命名；但这些名称不得作为普通用户可见文案。
- 点击“计算经济性”后立即展示粗粒度进度条，按“准备参数和方案汇总、读取电费单和电价曲线、计算电源侧和同一主体经济性、整理结果、保存当前项目结果、完成”推进；进度条用于运行感知，不承诺精确计时或后台任务状态。

边界：
- 本轮不改变经济性现金流、FIRR、同一主体税前模型、负荷侧收益、推荐排序、价格曲线或项目结果持久化口径。
- 当前仍是 Streamlit 同步计算；进度条只是前台反馈，不等同于正式后台队列、取消或重试能力。

### 2026-07-04 数据曲线首屏继续压缩与湖南 110kV 电费单默认值

用户指出 02 方案仿真页的数据曲线区越做越长，“本次上传识别结果”和“当前数据状态”继续放在首屏会挤占候选方案池与政策约束的空间；同时要求 03 经济性测算页“下网电费单”中的线损费用、输配电价、政府性基金及附加默认按湖南 110kV 工商业用户当前常用账单项预填，取四位小数，系统运行费因月度波动较大暂按 0.0500 元/kWh。

决策：
- 02 页数据曲线主面板只保留 Demo 开关、批量上传入口和一句短提示；“本次上传识别结果”“当前数据状态”、文件名、摘要、列识别和异常点统计统一放入“数据质量与识别详情”折叠区，用户需要复核时再展开。
- “数据质量与识别详情”继续定位为查阅识别和质量结果，不承诺在平台内编辑表格；数据问题仍应修改源 CSV/XLSX 后重新上传。
- 03 页“下网电费单”默认值按湖南 110kV 工商业两部制账单项预填：到户综合电价 0.6523 元/kWh，线损单价 0.0220 元/kWh，系统运行费 0.0500 元/kWh，输配电价 0.1104 元/kWh，政府基金及附加 0.0463 元/kWh。
- 上述默认值只是项目起始值，页面文案提示用户应按实际项目电费单复核；价格曲线模式仍优先使用用户明确上传的项目级下网电价曲线，固定电费单参数不覆盖曲线结果。

边界：
- 本轮不改变风光储技术仿真、曲线读取安全策略、经济性计算公式、价格曲线口径或推荐排序，只调整 UI 信息层级、默认值和测试。
- 系统运行费后续若需要更准确，应考虑从月度代理购电表或项目电费单读取，而不是长期固定为 0.0500。

### 2026-07-04 项目结果集收敛为“一个当前结果”

用户指出 01 项目启动页仍显示 3 个没有继续完成经济测算和图表生成的“结果版本”，实际容易被理解为一个项目保存了多个半成品结果。用户明确希望一个项目只保留一套当前结果：方案遍历 + 经济计算 + 推荐/图表/报告，图表应从已有方案计算和经济性结果按需生成，不应提前堆出多个结果版本。

决策：
- 项目首页从“运行记录 / 结果版本”数量展示改为“当前结果 / 交付产物”：普通用户默认只看到一个当前结果集，不再把多次技术仿真快照当成多个项目结果展示。
- 当前结果状态用一行聚合展示：方案遍历、经济测算、推荐方案、图表/报告；图表和报告默认显示“按需生成”，只有用户显式保存 HTML 图表包、Markdown/DOCX 报告等才计为交付产物。
- 重新运行方案遍历后，新技术仿真结果成为当前项目结果；旧 study 下的结果索引自动软隐藏，不从默认项目结果列表出现，但底层 artifact 文件不物理删除，仍保留审计和后续排查空间。
- 经济测算和推荐结果仍可以在底层作为独立 result record 写入，但 UI 必须按当前技术仿真 study 聚合成同一个结果集，不把技术、经济、推荐拆成多个普通用户要管理的结果版本。
- 完整 Job 流水保留在“运行审计（排查用）”折叠区，用于排查、审计、取消活动任务和管理员处理异常；它不是用户侧“项目有多个结果”的主语义。
- “当前结果恢复与交付产物”只面向当前结果集；自动保存的技术/经济/推荐 summary 是计算快照，不是交付产物。

边界：
- 本轮不改变风光储技术仿真、经济性测算、推荐排序或图表生成口径，只调整项目结果索引默认可见性、项目首页 UI 聚合和相关测试。
- 旧结果索引采用软隐藏，不做物理删除、磁盘清理或完整历史恢复工作流；若后续用户需要释放磁盘空间，应另做管理员确认的清理工具。

### 2026-07-04 项目任务区与数据曲线区首屏降噪

用户指出 01 项目启动页“任务状态明细”出现 2 条记录容易误解为创建了 2 个项目，且 `JOB ID`、`STUDY ID` 等内部编号对普通使用者没有默认展示价值；同时 02 方案仿真页 `Input 数据曲线` 上传说明仍过长，黄色警告时不应在首屏展示大段原因说明。

决策：
- “项目任务与结果”中的数量改称“运行记录”和“结果版本”，明确它们统计的是当前项目内的每次仿真、经济测算、推荐或导出运行，不是项目数量。
- 默认只展示最近一次运行和最近结果版本，表头全部使用中文；`任务编号`、`测算编号`、`结果编号` 仅放在“完整运行记录与结果版本”折叠区，用于排查、审计和恢复历史结果。
- 项目启动页把自动保存的仿真/经济/推荐结果称为“结果版本”或“计算结果快照”，不再把它们包装成“历史结果产物”；只有用户在导出页显式保存的 HTML 图表包、Markdown 报告、DOCX 报告模板等才作为“交付产物”展示。
- 技术仿真保存时仍可在底层附带输入曲线和配置快照，用于跨会话恢复和后续补算；这不等于用户上传曲线或调整参数就生成了交付产物。
- 本地项目侧边栏首屏不再展示内部项目编号，项目 ID 仅在“项目操作”折叠区作为排查信息保留。
- 保存成功提示不再显示 `Job / Result / Artifact` 英文内部编号，改为“已保存本次……到当前项目”的用户动作反馈。
- 任务表中的常见英文进度消息在显示层转为中文，例如 “technical study result ready” 显示为“方案仿真结果已就绪”。
- 02 页曲线上传入口压缩为“上传 / 更新曲线”；曲线单位说明放在上传控件的问号帮助中，不放进“数据质量与识别详情”。
- “当前数据状态”只展示各曲线状态胶囊，不默认展示文件名、摘要或黄色告警原因；文件名、摘要、识别列和异常点统计放进“数据质量与识别详情”。
- 当三条技术曲线可计算但存在空值、负值或光伏/风电标幺值大于 1 时，首屏只通过黄色状态提示“有警告”，不额外弹出高优先级说明；详细原因默认隐藏在详情折叠区。
- “数据质量与识别详情”定位为查阅识别结果和数据质量，不承诺在平台内编辑表格；若列识别或数据值有问题，应修改源 CSV/XLSX 后重新上传。

边界：
- 本轮只调整 UI 展示、文案和测试，不改变项目结果存储结构、Job/Study/Result 内部模型、风光储仿真、经济性测算或推荐排序口径。

### 2026-07-03 经济性页电费输入改为电费单友好口径

用户指出“外部购电净成本、下网到户价、绿电仍缴输配、绿电仍缴基金”对初入门者门槛过高，尤其外部购电净成本需要用户自行多轮计算。普通用户更容易从电费单拿到到户综合电价、输配电价、政府基金及附加、系统运行费、线损单价等明确项目，由软件内部推导净节费和仍缴费用更合理。

决策：
- 03 经济性测算页默认不再让用户直接填写“外部购电净成本”；默认改为“下网电费单”输入：到户综合电价、线损单价、系统运行费、输配电价、政府基金及附加、购电增值税率。
- 软件内部由到户综合电价反推电能量/市场购电价，并派生同一主体净节费和负荷侧可减少购网费用。
- 自发自用绿电默认仍缴输配电价和政府基金及附加，分别等于电费单中的输配电价和政府基金；若项目政策明确免缴或部分缴纳，在“高级：特殊电费口径”中覆盖。
- “直接输入同一主体净节费单价”和“单独覆盖负荷侧可减少购网费用单价”保留为专家复核入口，不作为普通用户默认路径。

边界：
- 本轮不改变同一主体 FIRR、负荷侧可成交收益、电源侧收益或推荐排序公式，只改变默认输入路径和 UI 文案。
- 价格曲线模式仍优先读取项目级下网电价曲线；已有曲线时，本页固定电费单参数和高级覆盖不覆盖曲线结果。

### 2026-07-03 数据曲线上传入口收敛与即时反馈

用户指出“高级：单独上传覆盖、数据识别复核、曲线识别摘要、可选：下网电价曲线”都堆在 `Input 数据曲线` 里显得过重；更合理的是保留一个批量上传入口，上传后就在当前页面即时反馈识别结果，若某条曲线有问题，再通过重新上传对应文件覆盖。

决策：
- 02 方案仿真页的数据曲线区只保留一个“批量上传 / 更新曲线文件”入口，可一次上传负荷、光伏、风电和可选下网电价曲线，也可只上传其中一条来更新。
- 曲线类型继续通过文件名关键词自动识别；技术曲线仍只纳入 CSV，下网电价曲线允许 CSV/XLSX/XLSM。
- 上传后同页展示“本次上传识别结果”和“当前数据状态”，用户能立即看到每个文件被识别为何种曲线、是否被纳入、必需曲线是否缺失，以及电价曲线是否作为可选输入存在。
- 时间列/数值列人工修正保留在“数据质量与识别详情”折叠区；不再单独保留“高级：单独上传覆盖”“曲线识别摘要”“可选：下网电价曲线”三个入口。

边界：
- 本轮只调整 02 页输入区交互与状态提示，不改变风光储技术仿真、下网电价曲线经济性口径、推荐排序或上传安全策略。
- 物理保存上传原始文件、价格曲线 artifact 化、历史项目恢复时的电价曲线恢复仍是后续 ResultStore 闭环任务。

### 2026-07-03 本地项目库创建体验优化

用户试用本地项目库后指出三点：项目 ID 不应让普通用户手填；侧边栏“当前项目”下拉框在深色主题中选中项目名不可读；已有项目后继续新建项目不应依赖退出重进。

决策：
- 项目 ID 改为软件自动生成，作为内部目录/索引用标识；用户创建项目时只填写项目名称。
- 本地项目库下拉框只显示项目名称，不把内部 ID、角色和导出权限塞进本地用户视图。
- 第一次创建项目仍在主页面完成；已有项目后，侧边栏保留“新建项目”入口，创建成功后自动切换到新项目；切换项目仍清空当前会话工作态，但保留项目历史。
- 侧边栏“项目操作”补“删除当前项目”入口；不要求用户重复输入项目名称。当前删除为软删除/归档，项目从本地列表隐藏，项目文件暂不物理删除。删除后若还有其他项目则自动切换到下一个项目；若没有项目则回到创建项目状态。
- 修正深色侧边栏里的输入框/下拉框字体颜色，确保当前项目名称可读。

边界：
- 本轮只优化项目库 UI、创建和软删除逻辑，不改变项目存储目录结构、权限模型、仿真计算、经济评价或推荐排序口径。
- 物理删除项目目录、释放磁盘空间、恢复已归档项目和项目重命名仍待后续补完整项目管理页。

### 2026-07-03 本地项目库与项目结果持久化

用户指出当前本地版每次跑完项目退出后容易丢失所有数据，且上线前也需要先补“创建项目 / 保存项目 / 恢复项目”的基础能力。担心点是每个项目若有约 300 个方案，全部保存是否会给服务器造成较高压力。

决策：
- 本地版默认启用本地项目库，不启用账号门禁时使用一个本地用户和 `.runtime/local_project_store`；启用内部试用登录时继续使用原 pilot 项目/权限体系。
- 打开本地版时先创建或选择项目；切换项目会清空当前会话结果，但不会删除项目历史。
- 项目默认保存轻量但可复核的结果：输入曲线 artifact、配置快照、技术方案汇总、经济性 summary、推荐组合和显式保存的报告/图表产物。
- 不默认保存所有候选方案的全年逐小时明细；推荐方案、当前查看方案或用户点选方案需要图表/报告/审计时，再按需加载、补算并保存对应单方案明细。
- 对 300 个方案量级，常驻保存 summary 和配置通常是 MB 级；真正影响存储的是“方案数 × 8760/8784 小时 × 字段数”的逐小时明细，因此必须继续坚持 summary-first 和按需明细策略。

边界：
- 本轮复用现有 `LocalPilotRegistry`、`LocalResultStore`、`PilotAccessService` 和 `pilot_study_persistence`，没有改变技术仿真、经济性计算或推荐排序口径。
- 本地项目库不是正式数据库；后续上线仍应迁移或适配 SQLite/Postgres、对象存储、备份恢复和后台 worker。

### 2026-07-03 公网内测账号注册与权限后台方向

用户确认首发不采用国内手机号注册，避免引入短信服务、手机号个人信息处理、隐私政策/用户协议和朋友备案网站材料变更等额外负担。更合适的路线是账号/密码或邀请注册，默认只授予低权限，高级功能由平台管理员后台开通。

决策：
- 首发账号体系不强制手机号，不接短信验证码；可以继续优先采用管理员创建账号/邀请码，也可在后续增加非手机号自助注册，但新注册用户必须默认低权限。
- 默认低权限用户可登录、创建或进入授权项目、查看页面结果，并可按后台授权下载基础技术汇总；经济评价详表、年度现金流、逐小时明细、图表包、DOCX/Markdown/ZIP 报告等高级产物默认关闭。
- 后台权限不应长期只有一个 `can_export_artifacts` 总开关；后续应演进为多个可组合功能项，例如基础汇总下载、逐小时明细下载、经济汇总下载、经济详表/现金流下载、图表包下载、报告下载、后台任务提交、项目成员管理等。
- 权限必须由后端服务校验并写审计日志，不能只隐藏前端按钮；未来 API、反向代理下载和对象存储签名 URL 也必须复用同一授权语义。
- 上线工作应和日常功能优化隔离：用稳定部署分支/标签、仓库外数据目录和备份恢复流程承载内测，开发分支继续迭代功能，测试通过后再合并到部署分支。

边界：
- 本轮只是产品和上线策略决策，不改变技术仿真、经济性计算、推荐排序或现有权限代码。
- 若后续重新考虑手机号注册，必须先补齐短信服务、隐私告知、用户协议、个人信息保存/删除机制和备案/公安备案影响确认。
...
### 2026-07-04 大批量测算进度与前端刷新开销收口

用户反馈 1000+ 候选方案下方案遍历和经济计算体感偏慢，且并行进程数改为非 1 时页面像是无反应；经济计算进度条只有粗粒度节点。

决策：
- 技术仿真页启动计算后立即展示“正在准备/正在启动并行进程”的进度文案，避免 Windows/Streamlit 多进程启动阶段看起来无响应；
- 技术仿真进度刷新改为约每 1% 或首尾方案刷新一次，避免 1000+ 场景时每个方案都向前端发送一次进度更新；
- 经济计算核心新增可选进度回调，电源侧经济性和同一主体经济性按同一总进度连续推进；
- UI 经济进度条从“准备、开始、整理、完成”升级为可显示当前计算阶段和已完成方案数的细分进度。

边界：
- 本轮不改变小时级技术调度、方案枚举、政策过滤、经济现金流和推荐排序口径；
- 并行进程数仍只作用于技术仿真，且 8760 小时大批量场景可能有收益，小样本或短曲线可能因进程启动开销反而更慢；
- 经济计算本轮只补进度反馈和少量回调开销控制，不引入并行经济计算。

### 2026-07-05 逐时电价经济性进度与经济页交互细节

用户在 2040 个方案、8784 行项目级下网电价曲线场景下反馈：经济计算几分钟没有具体百分比进度；大批量年度现金流提示中的“常驻”表述难懂；经济页造价/运维 `+/-` 微调反应偏慢；推荐卡片可点击后无法拖选复制文字；经济页顶部计算状态和计算按钮与左侧运行参数卡片不平齐。

决策：
- 逐小时电价曲线应用阶段纳入经济性总进度，经济计算总进度按“逐小时电价匹配 / 电源侧经济性 / 同一主体经济性”三段连续推进，并在进度条文本中显示总进度百分比；
- 逐小时电价曲线在同一项目小时顺序一致时只派生一次有效价格序列，各方案复用该价格序列进行加权聚合，减少 2000+ 方案下重复构造价格表的开销；
- 大批量经济性提示改为用户可理解的说法：全部方案都会计算经济性汇总、FIRR/NPV 和推荐排序，年度现金流明细只默认保存代表方案，其他方案按需补算；
- 经济页高频 `+/-` 微调改为表单提交前回调更新值，去掉按钮点击后的额外强制 rerun，降低单次点击等待感；
- 推荐卡片由透明按钮覆盖改为链接式卡片，通过 URL 参数同步当前查看方案，保留整卡点击的同时允许拖选复制卡片文字；
- 经济页顶部右侧改为一个独立边框面板，统一承载计算状态和“计算经济性”按钮，与左侧运行参数卡片顶部对齐。

边界：
- 本轮不改变技术仿真、储能调度、价格曲线字段含义、经济现金流公式、FIRR/NPV 口径或推荐排序算法；
- 年度现金流明细保存策略仍是结果包大小控制策略，不影响全量方案汇总和推荐排序；
- 推荐卡片链接依赖当前 Streamlit 会话状态，仍不是独立可分享的深链接。
