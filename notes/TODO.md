...
- Streamlit 欢迎页已新增只读“项目任务与结果”面板，可查看当前项目任务数、已保存结果数、最近任务和最近结果索引；完整历史结果恢复、artifact 下载、删除和后台任务状态页仍待实现。
...
- 下一阶段把历史结果恢复/下载/删除、经济性年度现金流、推荐跨会话去重、图表/报告导出也提交为项目级 `Job`，并把对应 hourly/cashflow/chart/report artifacts 写入 `ResultStore`。
