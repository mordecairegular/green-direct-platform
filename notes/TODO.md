...
- `src/green_direct/cli.py` 已新增 `pilot-admin` 命令行入口，支持 bootstrap、创建用户、重置密码、停用用户、授予/撤销平台管理员、列出用户/会话/审计事件/项目/项目成员/任务、认领 queued job、创建或归档项目、授予或禁用项目成员、清理过期 artifact payload 和标记超时 running 任务失败，作为管理员页面前的本地运维入口。
...
- 服务层已新增 `PilotAccessService`，把项目角色权限、可见项目列表、任务提交/取消、worker 认领、产物读取和审计日志统一成可测试服务门面，暂未包含 worker 执行器、数据库事务或并发锁。
