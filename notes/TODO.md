...
- 服务层已新增 `LocalPilotAdminService`，区分平台管理员和项目管理员，支持首个管理员 bootstrap、创建用户、重置密码并撤销有效会话、授予/撤销平台管理员、停用用户并撤销会话、恢复停用用户，也支持平台管理员创建/归档项目、查看项目、授予/禁用项目成员和只读查看审计事件；Streamlit 已接入最小平台管理页。
- `src/green_direct/cli.py` 已新增 `pilot-admin` 命令行入口，支持 bootstrap、创建用户、重置密码、停用/恢复用户、授予/撤销平台管理员、列出用户/会话/审计事件/项目/项目成员/任务、认领 queued job、刷新 worker heartbeat/进度、标记 worker 成功/失败终态、创建或归档项目、授予或禁用项目成员、清理过期 artifact payload 和标记超时 running 任务失败，作为管理员页面前的本地运维入口。
...
- Streamlit 主 UI 已新增最小“平台管理”页：平台管理员可创建账号、重置密码并撤销有效会话、停用/恢复账号、授予/撤销平台管理员、查看会话，并在“项目和成员”中创建/归档项目、维护项目成员角色，在“任务运维”中处理受支持 queued job / 恢复超时 running 任务元数据，在“审计日志”中只读查看全局或项目级审计事件；普通用户看不到该入口。
...
- 内部试用部署材料已新增第一版：`.env.example`、`docs/INTERNAL_PILOT_DEPLOYMENT_RUNBOOK.md`、`scripts/backup_pilot_store.ps1` 和 `scripts/restore_pilot_store.ps1`，覆盖环境变量、账号 bootstrap、启动、备份、恢复、过期清理、冒烟检查和回滚边界；`pilot-admin doctor` 已可在 bootstrap 前后检查 pilot store 目录、JSON metadata、payload 写入、协作锁和审计 JSONL，`preflight_internal_pilot_deploy.py --pilot-store-dir` 可把该检查合入源码树/VM 部署 preflight。
...
- 推荐页、图表概览页和导出/报告页缺少单方案逐小时明细时，已可在当前项目结果保存 `technical_summary`、`config_snapshot` 和三条 `input_curve_*` artifact 的前提下提交 `technical_study/hourly_detail` 后台 queued job；当前按需明细区域会轮询任务状态并在任务成功后加载 hourly artifact，当前会话仍有 `TechnicalStudyInput` 时保留同步补算按钮。
...
- 后台任务接入时以 `LocalJobStore` 的 `Job` 状态契约为临时边界，沿用 queued job 认领、`queue_job_with_input_artifact()`、`execute_next_worker_job()`、`input_artifact_ids`、`worker_id`、`last_heartbeat_at` 和 stale running cleanup 语义，再替换为 SQLite/Postgres 或正式队列实现；
