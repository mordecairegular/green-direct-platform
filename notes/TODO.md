...
- `src/green_direct/cli.py` 已新增 `pilot-admin` 命令行入口，支持 bootstrap、创建用户、重置密码、停用用户、授予/撤销平台管理员、列出用户/会话/项目/项目成员/任务、授予或禁用项目成员、清理过期 artifact payload 和标记超时 running 任务失败，作为管理员页面前的本地运维入口。
