...
- 下一阶段把完整历史结果恢复/删除/标记、经济性年度现金流、推荐工作台状态与跨会话去重、图表/报告导出也提交为项目级 `Job`，并把对应 hourly/cashflow/chart/report artifacts 写入 `ResultStore`。
