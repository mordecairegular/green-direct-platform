...
- 2026-06-15 已新增本地任务状态存储，并于 2026-06-16 补 worker/heartbeat 和卡死任务恢复语义：`src/green_direct/services/job_store.py` 提供 `LocalJobStore`，按 `project_id` / `study_id` / `job_id` 保存任务 JSON，支持提交、读取、项目/研究列表、状态筛选、进度更新、成功/失败/取消状态持久化；`Job` 模型已增加 `progress_current`、`progress_total`、`progress_message`、`worker_id` 和 `last_heartbeat_at`；`pilot-admin fail-stale-jobs` 可由平台管理员把超时 running 任务标记为 failed 并写 `COMPLETE_JOB` 审计。该 store 只保存任务元数据，不包含真正 worker、重试、资源中断、管理员 UI 或数据库锁；后续后台任务接入应沿用此状态契约。
