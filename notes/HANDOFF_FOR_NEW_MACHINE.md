...
受控公网内测 Route A 不是正式公网 SaaS：首发不采用手机号注册或短信验证码，优先由管理员创建账号或邀请码注册；如后续开放非手机号账号/密码自助注册，新用户必须默认低权限，禁止默认开放高级导出。软件不得接入 EMS、SCADA、调度自动化、真实电力设备或生产控制网络；公网访问前必须补独立导出权限、上传文件安全、项目/Run/Artifact 留存、审计日志、HTTPS/反向代理、备份恢复和回滚说明。

2026-07-03 账号与权限方向已确认：不为了首发引入手机号个人信息处理、短信服务、隐私政策/用户协议和朋友备案网站材料变更。后台权限应从当前 `can_export_artifacts` 总开关逐步演进为多个可组合功能项，至少区分基础技术汇总下载、逐小时明细下载、经济汇总下载、经济详表/年度现金流下载、图表包下载、报告下载、后台任务提交和项目成员管理。权限必须由后端服务校验并写审计，不能只隐藏前端按钮。

2026-07-03 已新增本地项目库方向并落地第一步：未启用 `GREEN_DIRECT_ENABLE_PILOT_AUTH` 时，Streamlit 默认启用 `GREEN_DIRECT_ENABLE_LOCAL_PROJECTS=1` 语义，使用本地用户 `local_user` 和 `.runtime/local_project_store` 复用现有项目/结果存储；打开本地版需创建或选择项目，项目结果会保存输入曲线、配置快照、技术 summary、经济 summary、推荐组合和显式保存的报告/图表产物。不要默认保存所有方案的全年逐小时明细；300 个方案量级应继续 summary-first，逐小时明细只对推荐/查看/导出方案按需保存。

2026-07-03 本地项目库创建体验已调整：项目 ID 由软件自动生成，用户只填写项目名称；本地项目下拉框只显示项目名称；第一次创建项目走主页面表单，已有项目后侧边栏保留“新建项目”入口，创建成功后自动切换到新项目；侧边栏“项目操作”已补“删除当前项目”，不要求用户重复输入项目名称，当前删除为软删除/归档，项目从列表隐藏但不物理删除项目目录，删除后自动切到剩余项目或回到创建项目状态；深色侧边栏的输入框/下拉框字体颜色已修正。后续不要再把内部 `project_id` 作为普通用户必填项暴露；若用户要求释放磁盘空间，应另做物理删除/清理流程，并补恢复已归档项目和项目重命名入口。

2026-07-03 起方案仿真页 `Input 数据曲线` 已收敛为一个“批量上传 / 更新曲线文件”入口：负荷、光伏、风电和可选下网电价曲线都从这里上传或覆盖。2026-07-04 已继续压缩首屏，“本次上传识别结果”和“当前数据状态”不再默认铺在上传区下方，而是统一收进“数据质量与识别详情”折叠区；人工列识别修正、文件名、摘要、识别列和异常点统计也都在该折叠区查阅。不要再恢复“高级：单独上传覆盖”“曲线识别摘要”或单独的下网电价曲线上传区，除非重新形成明确产品决策。

2026-07-04 已进一步压缩 01/02 页首屏信息：项目任务区默认按“一个当前结果集”展示，不再把多次自动技术仿真快照显示成多个普通用户要管理的结果版本；当前结果状态聚合展示方案遍历、经济测算、推荐方案和图表/报告，图表/报告默认按需生成，只有用户显式保存的 HTML 图表包、Markdown 报告、DOCX 报告模板等才作为“交付产物”计数。重新运行方案遍历后，新技术仿真结果成为当前项目结果，旧 study 下的结果索引自动软隐藏但不物理删除 artifact 文件；完整任务流水保留在“运行审计（排查用）”折叠区供排查、审计和恢复使用。不要在普通用户首屏重新暴露 `job_id` / `study_id` / `result_id` / `project_id`，常见任务进度消息也应显示为中文；自动保存的仿真/经济/推荐 summary 应称为计算快照或当前结果的一部分，不要包装成“历史结果产物”。02 页曲线上传入口文案改为“上传 / 更新曲线”，曲线单位说明放在上传控件问号帮助中；“数据质量与识别详情”用于查阅本次上传识别、当前数据状态、文件摘要、识别列和异常点统计，不承诺平台内编辑表格，问题数据应修改源 CSV/XLSX 后重新上传。

2026-07-03 经济性页默认电费输入已从“外部购电净成本”改为电费单友好口径：普通用户填写到户综合电价、线损单价、系统运行费、输配电价、政府基金及附加和购电增值税率，程序内部反推电能量/市场购电价，并派生同一主体净节费和负荷侧可减少购网费用。2026-07-04 下网电费单起始值按湖南 110kV 工商业两部制用户常用账单项预填：到户综合电价 0.6523 元/kWh，线损单价 0.0220 元/kWh，系统运行费 0.0500 元/kWh，输配电价 0.1104 元/kWh，政府基金及附加 0.0463 元/kWh；这些只是默认起点，用户仍应按项目电费单复核。自发自用绿电默认仍缴输配电价和政府基金；特殊免缴/部分缴纳、直接输入同一主体净节费、单独覆盖负荷侧可减少购网费用都在“高级：特殊电费口径”。不要再把“外部购电净成本”作为普通用户默认必填项。UI 上“下网电费单”应保持独立满宽卡片，六个账单项按三列两行铺满；说明文字放在标题问号提示中，不要在正文铺长段说明。

2026-07-04 经济性页对外 UI 已去掉“V1”等内部阶段名：按钮、成功提示、底部状态和用户快速指南统一使用“经济性测算 / 计算经济性 / 经济性已计算”。内部 session key、历史口径文档和开发资料仍可保留 `economy_v1_result` 等名称，但不得暴露给普通用户。点击“计算经济性”后会显示粗粒度进度条，阶段包括准备参数、读取电费单和电价曲线、计算经济性、整理结果、保存项目结果和完成；该进度条是同步 Streamlit 计算的运行反馈，不代表正式后台队列或精确进度。

2026-07-04 方案推荐页已把推荐卡片接入经济性对比图：整张推荐卡片就是点击目标，不再额外放“切换到此方案”按钮；点击卡片后卡片高亮，并同步更新经济性 3D 图中的红色光环和定位线。卡片视觉必须保持原 `.gd-rec-card` HTML 排版，包括序号、标签、方案号、状态胶囊、容量、推荐理由和指标栅格；不要再用纯文本 `st.button` 替代卡片展示。推荐页专用当前查看方案状态优先于图表页和导出页旧状态。经济性对比图 hover 已增加“推荐席位”，红色推荐菱形会显示命中的推荐方向，普通候选显示“普通候选”。该调整只改 04 页展示与交互，不改变推荐算法、经济性计算、政策筛选或导出口径。
...
- 2026-06-10 已进一步确认 UI 改造不能过度守旧：保留计算口径不等于保留旧页面结构。03 页经济参数已从单一“经济性参数工作台”大框拆为运行口径、建设投资、运维成本、收入和税金、到户电价展示、高级参数六个小工作卡；参数被放入 `st.form("economy_v1_params_form")`，对外提交按钮应称为“计算经济性”。表单内编辑常调参数时不再触发整页 rerun，浏览器测得风电单位造价输入改动前端响应约 82ms。后续 UI 工作应围绕“快速完成方案策划、经济测算、推荐和交付”主目标，不要为了沿用旧大表单而牺牲交互流畅性。
