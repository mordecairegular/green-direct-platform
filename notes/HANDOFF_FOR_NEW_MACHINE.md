...
- 2026-06-15 已新增 `pilot-admin` 命令行运维入口：`python -m green_direct.cli pilot-admin ...`，也可通过安装后的 `green-direct pilot-admin ...` 使用。支持 `bootstrap`、`create-user`、`reset-password`、`disable-user`、`grant-platform-admin`、`revoke-platform-admin`、`list-users` 和 `list-sessions`；密码优先用 `--password-env` 从环境变量读取。默认 store 为 `.runtime/pilot_store`，内部试用部署时应显式传 `--store-dir` 到受控目录。
