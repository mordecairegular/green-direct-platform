...
- 2026-06-15 已新增内部试用后台模型骨架，并于 2026-06-16 补任务输入 artifact 引用契约：`src/green_direct/models/pilot_backend.py` 定义 `User`、`Project`、`ProjectMembership`、`ProjectStudy`、`Job`、`JobArtifact`、`StudyResultRecord`、`AuditLog` 及角色/任务/产物状态枚举；`Job.input_artifact_ids` 用于后续 worker 从同一项目/研究的 `ResultStore` artifact 读取输入，不把大 payload 塞进 job JSON。该骨架暂未接入正式数据库、真正任务队列或完整后台管理页；下一步应基于它实现 worker wrapper、SQLite/Postgres 存储和正式历史结果管理。
