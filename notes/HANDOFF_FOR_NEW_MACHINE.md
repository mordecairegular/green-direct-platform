...
- `scripts/benchmark_internal_pilot_performance.py`：用于记录技术仿真和经济性测算的可重复 benchmark；经济性真实等待优先看 `--no-tracemalloc` 直计时，默认模式用于同时观察 Python heap 峰值。
