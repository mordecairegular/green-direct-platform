...
- 下一步建议继续把完整项目任务状态页、预计耗时/取消、历史结果恢复/下载/删除、缺失 hourly artifact 的跨会话补算、经济性年度现金流、导出页和图表模块从兼容层 `batch_result` / session_state 逐步迁移到 `StudyResult` + `ResultStore` 读取；完善 `recommendation/` 模块的数据模型和导出契约，并为典型日选择、图表包和报告输出补充更细的审计数据。
