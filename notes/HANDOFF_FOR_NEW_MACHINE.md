...
- 2026-06-17 已把方案数预估和硬上限拒绝改为 count-only 路径：`estimate_scenario_count()` 不再构造完整 `Scenario` 列表，`run_batch()` 会先拒绝超限方案池再物化可执行方案；`generate_scenarios()` 也预计算容量轴和储能功率/时长组合，减少大方案池启动前固定开销。25,596 个候选方案样本中，方案数预估约从 0.0407s 降到 0.0004s，方案对象生成约从 0.0410s 降到 0.0253s。该优化不改变 scenario_id 顺序、过滤规则或调度口径。
