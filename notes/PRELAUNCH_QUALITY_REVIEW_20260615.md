...
- 大批量汇总优先 summary-only 路径与可选并行技术仿真；
- 经济性批量评价性能优化；
...
1. 大批量汇总优先模式已能避免为未保留方案构造完整 `hourly_detail`，但仍只常驻保留前 N 个生成序号方案的逐小时明细。
   未保留方案会走 summary-only 技术仿真路径，已降低大批量筛选耗时和内存压力；但前 N 个方案不一定是推荐方案或经济排序靠前方案。后续应实现代表方案按需补算，或在排序后补保留推荐组合的逐小时明细。
