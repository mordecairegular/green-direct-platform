...
- 大批量汇总优先 summary-only 路径、当前会话单方案逐小时明细按需补算与可选并行技术仿真；
- 经济性批量评价性能优化；
...
- 本轮当前会话单方案逐小时明细按需补算 checkpoint 提交前，工作区为有意修改状态；提交后应重新确认干净。
...
1. 大批量汇总优先模式已能避免为未保留方案构造完整 `hourly_detail`，并已支持当前会话内按需补算单个方案逐小时明细。
   未保留方案会走 summary-only 技术仿真路径，已降低大批量筛选耗时和内存压力；推荐页、图表概览页和导出/报告页可对选中方案触发补算。但补算依赖当前 session 中未持久化的 `TechnicalStudyInput`，历史 summary-only 恢复后仍不能直接补算，也未写入项目级 hourly artifact。
...
6. 设计并实现下一阶段最小闭环：完整任务状态页 + 历史结果恢复/下载/删除 + 项目级 hourly artifact / 跨会话按需补算 + 图表/报告项目级 artifacts + 部署 runbook。
