...
UI 口径：价格曲线在“方案仿真”页作为可选项目级输入上传，只有用户在当前会话明确上传后才写入 `project_price_curve_data` 并进入“经济性测算”；运行时快照不恢复旧价格曲线，避免用户只上传负荷、光伏、风电三条技术曲线时误用历史电价。已有当前上传的项目级曲线时，固定外部购电净成本、电费清单组价和负荷侧单独覆盖控件应置灰，防止同一条下网节费口径被重复覆盖。
