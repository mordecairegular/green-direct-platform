...
| `wind_capex_per_kw_with_vat` | 5000 | 元/kW，含税 | 风电单位造价 | 电源侧、同一主体、推荐投资估算 |
| `pv_capex_per_kw_with_vat` | 2800 | 元/kW，含税 | 光伏单位造价 | 电源侧、同一主体、推荐投资估算 |
...
2026-07-03 后，固定价模式下 UI 默认不再要求普通用户直接填写“外部购电净成本单价”。默认输入改为电费单可直接读取的到户综合电价和账单项，程序内部倒推电能量/市场购电价，并派生同一主体税前净节费和负荷侧可减少购网费用。若需要更准确地区分特殊政策口径，用户有两条高级路径：

1. 覆盖“自用绿电仍缴输配/基金”，用于免缴或部分缴纳政策；
2. 在高级选项中单独覆盖“负荷侧可减少购网费用单价”。
3. 专家复核时可直接输入“同一主体净节费单价”，对应内部 `net_avoided_grid_cost_price`。

2026-07-04 起，Streamlit 03 页下网电费单的固定价模式 UI 起始值按湖南 110kV 工商业两部制用户常用账单项预填：到户综合电价 0.6523 元/kWh、线损单价 0.0220 元/kWh、系统运行费 0.0500 元/kWh、输配电价 0.1104 元/kWh、政府基金及附加 0.0463 元/kWh。该起始值只用于降低新项目录入门槛，不改变 `AvoidedGridPurchaseParams` 的模型默认值；用户应按项目电费单或已上传的逐时下网电价曲线复核。

| UI 输入 | 进入字段 | 用途 |
|---|---|---|
| 到户综合电价 | 先反推 `energy_market_price_with_vat`，并作为用能侧结算展示的下网路径价格 | 固定价模式默认输入 |
| 线损单价、系统运行费、输配电价、政府基金及附加 | `line_loss_price_with_vat`、`system_operation_fee_with_vat`、`transmission_distribution_tariff_with_vat`、`gov_fund_surcharge` | 派生同一主体净节费和负荷侧现金口径 |
| 自用绿电仍缴输配/基金（高级覆盖） | `green_direct_retained_transmission_distribution_tariff_with_vat`、`green_direct_retained_gov_fund_surcharge` | 特殊政策扣减；默认等于电费单中的输配电价和政府基金 |
| 同一主体净节费单价（高级覆盖） | `net_avoided_grid_cost_price` | 专家直接覆盖同一主体 FIRR 口径 |
| 负荷侧可减少购网费用单价（高级覆盖） | `load_side_avoided_charge_price` | 负荷侧可成交收益；默认由账单项内部推导 |
...
UI 口径：价格曲线在“方案仿真”页作为可选项目级输入上传，只有用户在当前会话明确上传后才写入 `project_price_curve_data` 并进入“经济性测算”；运行时快照不恢复旧价格曲线，避免用户只上传负荷、光伏、风电三条技术曲线时误用历史电价。已有当前上传的项目级曲线时，本页固定电费单输入、特殊电费口径覆盖和负荷侧单独覆盖控件应置灰，防止同一条下网节费口径被重复覆盖。
