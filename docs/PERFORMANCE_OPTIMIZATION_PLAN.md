...
- `run_batch()` 已改为通过 `iter_scenarios()` 流式消费方案池，串行和并行路径都不再先物化完整 `Scenario` list；并行进程池按 chunk 从 iterator 拉取方案，仍保持 `scenario_id`、progress total 和结果顺序稳定；
...
- 批量执行阶段避免先物化完整方案池；（已完成第三版：`iter_scenarios()` + 流式 chunk）
