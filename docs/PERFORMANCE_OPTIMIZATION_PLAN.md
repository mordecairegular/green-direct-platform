...
- 无储能且 `retain_hourly_detail=False` 的方案已接入 NumPy summary-only 快路径。该类方案没有 SOC 滚动状态，不需要逐小时创建 dispatch 调用；快路径直接按数组计算直供、下网、上网、弃电、年上网比例 cap、站用电和电网交换限额汇总，再复用 `calculate_summary_from_values()`，仅影响无储能 summary-only 分支，不改变完整逐小时明细或有储能口径。
...
- 已减少单方案热路径里的 `DispatchStep` 对象创建：`dispatch_hour_values_with_limits()` 与 `dispatch_hour_with_limits()` 保持同一计算逻辑，现有 golden/批量一致性测试用于证明口径不变；已将批量 runner 不消费的逐方案 diagnostics 变为可跳过，并把 hourly numeric cleanup 前移到 numpy 数组；无储能 summary-only 场景已走 NumPy 快路径；后续可继续评估有储能 summary-only 的更紧凑累加结构或编译化内核。
