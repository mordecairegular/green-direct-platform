...
- `LocalJobStore.claim_next_queued_job()`、`PilotAccessService.claim_next_job_for_worker()` 和 `pilot-admin claim-next-job` 已提供第一版 worker 认领原语，可按任务类型认领 queued job，写入 worker/heartbeat，并在服务层跳过归档项目。
