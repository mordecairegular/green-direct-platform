...
目标是支持内部 10-20 人试用，不是正式公网 SaaS。请先阅读 `src/green_direct/models/pilot_backend.py`、`src/green_direct/services/result_store.py`、`src/green_direct/services/pilot_registry.py` 和 `src/green_direct/services/job_store.py`，把它们视为已经落地的第一步模型、本地文件版 ResultStore、本地账户/项目注册表和本地任务状态存储，再基于当前 Streamlit 前台和 Python 服务层，设计下一阶段最小后台能力：
