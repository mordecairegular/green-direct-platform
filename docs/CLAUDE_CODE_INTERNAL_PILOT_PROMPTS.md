...
如果目标是“让 Claude Code 对 UI 进行提升”，不要直接发送一句“请美化界面”。应先让它用截图和浏览器证据审查当前 01-06 页，再选择一个可验收小切片落地。当前公网试用目标是同事用电脑端浏览器、通过移动网络/非公司内网访问；首次 UI 提升优先考虑桌面端完整工作流、经济性/推荐/导出页的任务状态反馈。窄屏/手机宽度可用性可作为兼容性改进，但不是本轮上线阻塞项。
...
然后用浏览器实际检查 01-06 页，至少覆盖桌面宽屏；如果本轮改动涉及响应式布局，再额外检查窄屏/手机宽度。若当前环境无法打开浏览器，请说明限制，并基于已有截图和代码审查，不要臆造浏览器已通过。历史截图只能作为线索，不能替代当前运行截图；如果代码已变更，应重新截图或明确“未重新截图”。
...
重点先判断这些已知问题是否仍存在：
1. 桌面端 01-06 页主 CTA、状态条、项目切换和页面跳转是否能支撑完整试用闭环；
...
首轮可选小切片建议：
1. 桌面端完整工作流：让同事通过移动网络访问时至少能完成登录、选择/创建项目和 Demo 试算；
...
4. 用浏览器实际打开本地 Streamlit，检查桌面端六个页面；如果改动涉及响应式布局，再检查窄屏/手机宽度；
5. 截图或文字记录检查结果；
...
2. 如果本轮改动涉及响应式布局，窄屏/手机宽度打开至少 01 项目启动、02 方案仿真和本轮改动页，确认没有导航、状态条、按钮或卡片遮挡主内容；
