...
如果目标只是让同事用电脑通过移动网络/非公司内网先试用，请优先按 `docs/PUBLIC_BETA_FIRST_LAUNCH_PLAYBOOK.md` 执行首次发布，再用 `docs/MOBILE_NETWORK_TRIAL_CHECKLIST.md` 做发给同事前验收；本文保留更完整的平台判断和架构边界。
