...
| 关键操作审计日志 | 部分满足 | 登录、项目、成员、任务、artifact 写入/网页查看/下载/清理、结果索引标记和软删除、当前 06 页临时导出下载已审计 | 管理员跨项目查看、原始文件查看、未来 API/反向代理下载仍需补齐 | 扩充 `AuditAction` 覆盖面 |
...
| 核心算法回归通过 | 满足当前 checkpoint | 最近 `pytest -q` 为 311 passed | 后续改性能/后台时仍需重复验证 | 每个工程化切片后跑回归 |
