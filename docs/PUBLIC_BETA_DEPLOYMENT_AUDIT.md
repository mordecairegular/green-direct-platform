...
- 计算仍主要在 Streamlit 进程内同步执行；已有活动任务查看/取消入口、worker 认领原语、按需 hourly detail / annual cashflow queued job 入口、one-shot worker、最小轮询 worker、stale running 恢复和 failed/canceled 手动重试，但没有正式队列、跨进程队列锁、worker 级取消和自动重试策略；
...
| 核心算法回归通过 | 满足当前 checkpoint | 最近 `python -m pytest -q` 为 362 passed | 后续改性能/后台时仍需重复验证 | 每个工程化切片后跑回归 |
