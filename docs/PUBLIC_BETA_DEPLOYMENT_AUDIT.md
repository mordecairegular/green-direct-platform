...
- 大方案池 summary-first、可选并行、当前会话单方案逐小时明细补算、hourly artifact 留存和已有 hourly artifact 跨会话加载；
...
| 核心算法回归通过 | 满足当前 checkpoint | 最近 `pytest -q` 为 284 passed | 后续改性能/后台时仍需重复验证 | 每个工程化切片后跑回归 |
