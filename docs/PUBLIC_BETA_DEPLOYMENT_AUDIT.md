...
- `pilot-admin doctor` 运行时 store 自检入口，可在 bootstrap 前后检查持久目录、JSON metadata、payload 写入、协作锁和审计 JSONL。
...
| Docker 可部署 | 第一版满足 | `Dockerfile`、`docker-compose.yml`、`README_DEPLOY.md`、`render.yaml`、`.github/workflows/internal-pilot-quality.yml`、`docs/MANAGED_PUBLIC_BETA_DEPLOYMENT.md`、`pilot-admin doctor` | 尚未在目标托管平台完成构建/启动/恢复演练；GitHub Actions 需要推送后在远端实际跑通；持久盘、套餐、区域和备份能力需在平台控制台确认 | 实机运行 `docker compose build/up` 或 Render Blueprint 部署，确认 GitHub Actions 质量门通过，运行 `pilot-admin doctor`，并完成数据卷恢复演练 |
...
| 核心算法回归通过 | 满足当前 checkpoint | 最近 `python -m pytest -q` 为 371 passed | 后续改性能/后台时仍需重复验证 | 每个工程化切片后跑回归 |
