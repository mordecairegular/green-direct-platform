...
| 核心算法回归通过 | 满足当前 checkpoint | 最近 `python -m pytest -q` 为 347 passed | 后续改性能/后台时仍需重复验证 | 每个工程化切片后跑回归 |
