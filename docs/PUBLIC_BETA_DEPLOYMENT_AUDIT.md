...
| 项目/Run/参数/结果摘要留存 | 部分满足 | 技术/经济/推荐 summary、推荐席位输入与按需 hourly artifact 已写 `ResultStore`；技术 summary、经济 summary、推荐席位输入和推荐 portfolio 可恢复到当前会话 | 年度现金流、推荐视角选择/重新排序工作台状态、图表和报告未完整持久化 | 按 `StudyResultRecord` 串联完整结果索引并补 cashflow/chart/report artifacts |
...
3. **历史结果恢复**
   - 从 `StudyResultRecord` 恢复技术 summary、经济 summary、推荐席位输入和推荐 portfolio；推荐视角选择与重新排序状态后续再恢复为完整工作台状态；
