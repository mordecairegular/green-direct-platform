...
- 计算仍主要在 Streamlit 进程内同步执行；已有活动任务查看/取消入口、worker 认领原语和第一条按需 hourly detail one-shot worker，但没有常驻后台 worker、跨进程队列锁、worker 级取消和重试闭环；
