...
| 管理员创建/停用用户、创建/归档项目和维护项目成员 | 第一版满足 | `LocalPilotAdminService`、`pilot-admin` 用户/项目/成员命令、Streamlit 平台管理页可创建/归档项目并维护成员 | 仍是本地文件版账号后台 | 后续迁移 SQLite/Postgres 或统一身份 |
...
| 核心算法回归通过 | 满足当前 checkpoint | 最近 `python -m pytest -q` 为 353 passed | 后续改性能/后台时仍需重复验证 | 每个工程化切片后跑回归 |
