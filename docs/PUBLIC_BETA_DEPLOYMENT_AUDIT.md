...
- `pilot-admin doctor` 运行时 store 自检入口，可在 bootstrap 前后检查持久目录、JSON metadata、payload 写入、协作锁和审计 JSONL；`preflight_internal_pilot_deploy.py --pilot-store-dir` 可把该检查并入源码树/VM 部署 preflight。
...
| Docker 可部署 | 第一版满足 | `Dockerfile`、`docker-compose.yml`、`README_DEPLOY.md`、`render.yaml`、`.github/workflows/internal-pilot-quality.yml`、`docs/MANAGED_PUBLIC_BETA_DEPLOYMENT.md`、`pilot-admin doctor`、`preflight_internal_pilot_deploy.py --pilot-store-dir` | 尚未在目标托管平台完成构建/启动/恢复演练；GitHub Actions 需要推送后在远端实际跑通；持久盘、套餐、区域和备份能力需在平台控制台确认 | 实机运行 `docker compose build/up` 或 Render Blueprint 部署，确认 GitHub Actions 质量门通过，运行 `pilot-admin doctor` 或 preflight store doctor，并完成数据卷恢复演练 |
